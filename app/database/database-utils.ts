...
export async function createIndexIfNotExists(db: Database, collectionName: string, indexSpec: string | string[], unique = false) {
  ...
  if (Array.isArray(indexSpec)) {
    console.log(`Creating unique index on fields '${indexSpec.join(", ")}' in collection ${collectionName}`);
  } else {
